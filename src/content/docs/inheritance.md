...
Переключатель над списком токенов («Скрыть доли» / «Раскрыть доли») задаёт, как доли попадают в скачанные файлы. Сейчас доступен только режим **«Раскрыть доли»**: в файлах наследников сохраняются полные доли по каждому токену (sharesBps). Режим «Скрыть доли» планируется в будущем.
