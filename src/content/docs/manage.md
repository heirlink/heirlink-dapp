...
Адрес vault возвращается фабрикой по адресу владельца. Хранилище создаётся при первой транзакции после нажатия "Создать хранилище" на странице "Наследство" (вызов `deployVault`). При следующих генерациях обновляется уже существующий vault (`updateWill`).
